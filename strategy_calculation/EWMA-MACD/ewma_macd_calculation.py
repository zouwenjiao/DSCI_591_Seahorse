...
def filter_signals_EWMA_MACD(trade_signals, data_path, ewma_window, mfast, mslow, msignal):
    """
    Generates the filter signals for the EWMA-MACD strategy given input parameters for a given stock.
    ...
    ewma_window : int
        Parameter value for exponential weighted moving average operation. Represents the intra-day intervals to be used. 
    mfast : int
    ...
    # EWMA, MACD
    ##############################################################
    # Calculating exponential moving average X
    ...
    filter_boolean = (df_signal["oscillator"] * df_signal["action"] < 0) &\
                ...
                (df_signal["action"]*df_signal["ewma_price_gradient"] > 0) &\
...
def portfolio_eval_ewma_macd(trade_signals, directory_path, ewma_window, mfast, mslow, msignal):
    """
    Compiles the filter signals for the EWMA-MACD strategy given input parameters for all stocks, and calculates the 
    ...
    ewma_window : int
        Parameter value for exponentially weighted moving average operation. Represents the intra-day intervals to be used. 
    mfast : int
        Parameter value for MACD Fast exponentially weighted moving average series. Represents day counts and not intra-day intervals.
    mslow : int
        Parameter value for MACD Slow exponentially weighted moving average series.  Represents day counts and not intra-day intervals.
    ...
    for i in range(len(files_list)):
        ...
        if files_list[i] in ["algn20years.txt","wynn20years.txt","klacl20years.txt","lvs20years.txt"]:
            ...
            if i == 0:
                compiled_df = filter_signals_EWMA_MACD(trade_signals, data_path, ewma_window, mfast, mslow, msignal)
                
            else:
                intermediate_df = filter_signals_EWMA_MACD(trade_signals, data_path, ewma_window, mfast, mslow, msignal)
...
###########################################################################
def strategy_eval_ewma_macd(trade_signals, directory_path, ewma_params_list, macd_params_dict):
    """
    Compiles the average annualised return metric on a yearly breakdown assuming equal stock weightage basket for multiple EWMA-MACD parameter combinations.
    ...
    directory_path : string
        String input for relative directory path location where all stocks data are kept.
    ewma_params_list: list
        List of EWMA window parameters (integer) for moving average operation. Represents the intra-day intervals to be used. 
    ...
    -------
    pandas.DataFrame object
        DataFrame object with yearly breakdown of average annualised return metric for multiple EWMA-MACD parameter combinations.
    """
    # Dictionary for storing data
    strategy_yearly_results = {}
    
    for i in tqdm(range(len(ewma_params_list))):
        ewma_window = ewma_params_list[i]
        
        for macd_combi in range(len(macd_params_dict)):
            # Create strategy_name for key
            strategy_name = "ewma("+str(ewma_window) + "/13)_macd("+str(macd_params_dict[macd_combi]["mfast"]) + ","+str(macd_params_dict[macd_combi]["mslow"])+","+str(macd_params_dict[macd_combi]["msignal"])+")"
            
            # Store yearly results 
            strategy_yearly_results[strategy_name] = portfolio_eval_ewma_macd(trade_signals, directory_path, ewma_window, 
...
def strategy_statistics_EWMA_MACD(trade_signals, directory_path, ewma_window, mfast, mslow, msignal):
    """
    Calculate relevant statistics for a given EWMA-MACD strategy.
...
def get_stats(data, trade_signals, directory_path):
    ...
    for i in tqdm(range(len(top_strategy_name_list))):
        ...
        = strategy_statistics_EWMA_MACD(trade_signals, directory_path, 
                                        ma_window,
                                        mfast,
                                        mslow,
                                        msignal)
